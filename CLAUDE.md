...
- **Sessions run in parallel: across modes, and more than one on a project.** Two Faber terminals on one repository are one Repo module naming both; nothing arbitrates between them beyond the vault's single write lock and git, so two sessions on one job split the work by file.
