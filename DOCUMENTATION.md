...
|---|---|---|---|---|
| General | Opus 5 | none, by design | none | built-ins only |
...
| Maintenance | Haiku 4.5 | `maintenance/audit.md` | `maintenance` | none |
...
| `worklist` | What is in flight across every mode: each job's stage, status and last touch, read from the job folders (the name predates the brief's retirement; it never read a worklist file) |
...
**The launcher** (`⌘T`) is where a session is entered: the mode, the directory it starts in, and how hard it thinks. The effort chooser opens on what that mode's model is already configured to do, read from `~/.claude/settings.json` through `GET /v2/mode-defaults`, so leaving it alone sends an override identical to the default; `interactive.py` refuses an unknown level before the terminal opens, because the CLI rejects it after the pane has painted and the error lands where a session should be. Each mode is its character's sprite, the same mark the tab strip and the Repo view use. The opening-prompt box went on 2026-09-16, for the reason the morning brief did: a fresh session opens by saying what it is, and anything else is a first message better typed into the terminal about to take focus. A handoff's carried summary is a different thing and stays.
...
**What the engine refused is read from the transcripts** (2026-09-16). `GET /v2/sessions/limits` carries `refused`: every model the engine is currently declining, with its label, the time, and the message it gave, cleared when that same model answers again. It exists because the windows cannot answer "can I work right now": Fable 5.1 returned 429 "You're out of usage credits" while the 7-day window read 51%, and the interface showed nothing, because its one alarm read a `using_overage` key the status line has never sent. That field is gone. Each transcript is read forward from where the last pass stopped, and the state is kept per file, because the refusal record is synthetic and the model comes from the turn before it: a tail window held the 429 and had already dropped that turn, so the banner appeared and vanished within the hour while the refusal was still current.
...
**Every run is on record** (`nightshift/report.py` → `backend/data/nightshift.json`): when it ran, what staged, what failed and why, sixty nights kept. `summary()` names the newest run's outcome, worst first: `broken` when every item failed with one error, `partial`, `dropped`, `staged`, `quiet`, and how many nights in a row have ended the same way; Settings' Maintenance section prints it as one sentence (`GET /v2/nightshift`), in Faber's red when the night was broken.

**`dropped` is the outcome added on 2026-09-17, and it outranks `staged`.** Three gates stand between Advance and Stage, each a real contract from `maintenance/inbox/README.md`: the drafter returned nothing, the draft has no `## Rationale`, its `## Confidence` is missing or is not high/low. All three were a bare `continue`, so an item that reached Advance, paid for a distiller call and then failed one of them counted in `seen`, appeared in neither `staged` nor `failed`, and the night reported itself quiet. The record holds `seen: 3, staged: 0, failed: 0` for 2026-09-16 and `seen: 4, staged: 1, failed: 0` for 09-17: six discarded drafts, six model calls, no reason and no text kept. Each gate now names itself in `drops`, and the rejected draft is moved to `backend/runtime/nightshift-drops/<slug>.md` rather than unlinked, so the morning has the artefact as well as the reason. `quiet` now means nothing was *seen*; it used to mean nothing was staged, which is the definition that let three drops read as a quiet night. **There is deliberately no fourth gate for a diff that cannot be applied** *(2026-09-18)*: it could be checked at draft time and dropped, and it is staged instead, because a dead proposal is readable in the Inbox with the accept route's own reason under it and can still be judged on its argument, where a drop is a line in a record and a file under `runtime/`. Entries written before the change carry no `dropped` key, so `drops_of()` infers it from `seen - staged - failed`, and both nights already on record read `dropped: 3`. The runner itself exits non-zero and says "FAILED, every advance failed: …" when the machine is broken, rather than "quiet night". This is the monitoring the six silent weeks lacked: a log that says "quiet" every night is unverified, not fine.

**The drafter returns its proposal; the runner writes it** *(2026-09-18)*. Advance used to hand the distiller a `Write` scoped to the one inbox path it was allowed to create, and every draft it ever produced was refused at that last step: any parenthesised specifier on `Write` in `--allowedTools` is denied, measured across `Write(/abs)`, `Write(//abs)`, `Write(rel)` and `Write(dir/**)`, and only a bare `Write` is honoured. So the model read the lessons files, found the pattern, wrote a correct four-part proposal, was denied, and the night recorded `no-draft`: three nights, nine model calls, nine finished proposals thrown away, about $1.30. The reason was in the subprocess's own JSON the entire time, under `permission_denials`, naming the tool and the exact path, and `capture_output=True` with nothing reading `stdout` discarded it. Advance is now `(item, vault_path) -> str`: the drafter runs on `Read Grep` with `Write` explicitly disallowed, hands its text back, and the runner writes the file after the gates pass, so a rejected draft never reaches `inbox/` at all and a passing one is written by the one writer that was always supposed to own it (`maintenance/agents/distiller.md` says this subagent is read-only, and now it literally is). `_result_text` reads the envelope: `is_error` raises before the text is trusted, a denial that left nothing behind raises after it, and both land in `failed` with their reason rather than in `dropped` with none. A fenced answer is unwrapped and a "Here is the proposal:" preamble cut, since that line would otherwise be a vault file's first line for good; nothing else is repaired, so a draft with no `## Rationale` still reaches the gate whole and the drops folder keeps it. `stdin` is `DEVNULL`, which saves the three-second wait per call for input that was never coming. The first live run staged three proposals. Two hardening fixes came out of reading real drafts: the first live drafter call wrote its header as `--- a/modes/dev/dev.md`, git's form rather than the format example's bare path, so `apply_proposal` strips an `a/`/`b/` prefix (the three staged proposals used the bare form, so this is defensive rather than a failure that happened), and a target the vault does not have is a 409 naming the file rather than a bare `FileNotFoundError` reaching the route as a 500.

**The prompt regression suite** (`prompts/regression.py`; `prompts/run_regression.py` is its command line; the Settings card is its face). `second-brain/prompts/regression.jsonl` holds one case per line, a mode, a prompt, a deterministic string assertion (`absent` strings must not appear; `contains` is any-of) and a `rule` naming the behaviour it guards: attribution, confusion-protocol, plan-before-code, drift-detection, propose-never-apply, verdict-reasoning, depth-over-breadth, commit-first-log-second, no-fabricated-decisions, state-vs-knowledge, mode-identity. A case runs as one `claude -p` session with the mode's composed prompt in `--append-system-prompt`, exactly as a hosted session receives it, in a neutral working directory so no project `CLAUDE.md` competes. **Scope follows composition:** `cases_for("faber")` is Faber's cases because the overlay is composed into one mode; `cases_for("system")` is every case because `system.md` is composed into all; `rule:<name>` crosses modes. **A failing case reruns once** before it counts. **Three run at a time.** **Each result is recorded** in `backend/data/regression.json` with the hash of everything that could change its answer: the composed prompt (`system.md` plus the mode's overlay) **and the methodology that overlay points at**. The methodology was outside the hash until 2026-09-17, because a session reads it on demand rather than being handed it at startup, so editing `dev.md` left every case reading "current" while `plan-before-code`, `confusion-protocol` and `commit-first-log-second` all test rules that live in exactly that file. A mode with no methodology (General, by design) is unaffected by its absence, and a mode that gains one goes stale, which is the right way round. `status()` marks a result `stale` when any of those inputs has since changed, so the card's tally, pass · fail · stale · never run, is a true state of the record, not of the last ceremony. Routes: `GET /v2/regression` (cases with last results, scope costs; never runs anything), `POST /v2/regression/run {scope}` (one run at a time, in a thread; 409 while one is going), `GET /v2/regression/status` (progress and results as they land). The scope is a row of words with a travelling outline, not a `select`: the scopes are six short words that all matter to the sentence around them, and a native menu hid the choice behind a click and drew macOS's own chrome in the middle of the card. The mark is a one-pixel ring in the signature rather than the filled pill the Prompts tabs use: that row is the whole control, this one sits inside a sentence, and a block of colour mid-line reads as a second background rather than as a choice. It is moved by `transform` and sized by `width` over 300ms, so it travels rather than teleporting and never relayouts the sentence around it. Hover moves the mark and nothing else; each scope carries its own cost as a tooltip, and the sentence and the button both describe the selection, because a sentence following the pointer read "affects 4 sessions" beside a button saying "run 13". The scope **`not current`** is offered first whenever it is non-empty: the stale results plus the cases never run, which is the run worth spending on after an edit, since re-proving the cases the edit could not have touched costs sessions to learn nothing. The card asks for the status **on mount**, not only after its own click: the run belongs to the backend and outlives the card, and while the card only knew about runs it had started, leaving Settings and coming back showed a Run button beside a suite that was running (2026-09-16). It polls every two seconds while a run is going, counts results as they land rather than at the end, and says whether the record is current and when it last ran. **First real run: 2026-09-16, 13 of 13 pass.**

**Audit → Propose → Apply, propose-never-commit.** Every proposal is accepted or rejected by hand, from Settings' Maintenance section (the Inbox tab until 2026-09-15; the routes keep the name). Nightshift's drafts land in `maintenance/inbox/`, where that section reads; until 2026-09-16 they went to `modes/nightshift/inbox/`, a path nothing had read since the cutover, so a night's proposal could be indexed and never shown. `POST /v2/inbox/{id}/accept` applies the diff *before* archiving, all hunks or none, then honours the proposal's markers (a lessons cursor to advance, a job to close), drops the entry from `maintenance/state.md`'s index and commits the vault; a diff that no longer applies is a 409 with the reason and the item stays, visibly stale. `reject` archives, drops the entry and commits. The listing joins each file with its index entry (description, rationale, confidence) and derives from the diff what accepting does, which file, how many places, so the row states the consequence before the argument; `full` carries rationale, diff, evidence and confidence for the read.
...
**The staleness pass runs first.** `staleness.flag_pass()` is nightshift's opening step: for every job folder in `jobs.JOB_FOLDERS` (`modes/<dev|learn|research>/jobs/` and `maintenance/jobs/`, one map shared with the Inbox's listing and the acknowledge route, so what can be flagged is exactly what can be seen and cleared) it marks a job `flagged` in its own `context.md`. Four facts, and the last two carry the weight: a session worked the job (a runtime log exists), it stopped without writing `SESSION_END`, that was longer ago than the six-hour threshold, and **nobody has written the job's own record since**. The log is the record of sessions; `last_touched` is the record of the job, written by whoever last looked at it. Taking the later of the two was the bug: on its first run (2026-09-17) the pass flagged two jobs whose sessions had ended without a clean close in July and August and whose status had been updated in September to say they were deliberately on hold. A job at Ship or Done is never flagged, and `flag_acknowledged_at` is the same statement made from the Inbox by a button rather than by an edit. Two logs count for one job, since a hosted session logs as `faber__<slug>` and a project's own hooks as `dev__<slug>`, and the newest **timestamped** line decides. Timestamped because an entry is `<time> <tool> <summary>` and a summary keeps the newlines its tool call had, so every multi-line Bash command leaves lines carrying no timestamp: 523 of 1705 in this project's own log on 2026-09-18. `_log_tail` read `lines[-1]` blind, so a log that happened to end mid-entry parsed as "no session ever worked this job" and the read fell through to the job's other log, which flagged this project as abandoned at 11:30 that morning while the session writing the offending heredoc was live in it. It reads back to the last line that parses now.

**Nightshift does not write about a flagged job.** It stages a proposal for undistilled lessons and nothing else. A `flagged-job` note existed until 2026-09-17: it proposed nothing, said so in its own text, appeared beside the same job in the Inbox's flagged list, and came back the night after it was accepted, because dedup only checks what is still pending. The Inbox reads the flags directly, and `POST /v2/flagged/{mode}/{slug}/acknowledge` clears one. It ran on v1's `GET /mode/{name}` poll until the 2026-09-12 cutover deleted the route; from then to 2026-09-16 nothing called it, and the scan read the `jobs` array in `modes/dev/state.md`, which nothing had written since v1 either. The folders are the only index now; the MCP `worklist` reads them too.
...
|---|---|---|---|
| `VAULT_PATH` | **yes** | none | Vault root, absolute. Every durable read and write goes here. |
| `NOCTIS_API_TOKEN` | **yes** | none | Bearer token on every route except `/health`. |
| `VITE_API_TOKEN` | **yes** | none | The same token, for the shell. Vite inlines it into the bundle, acceptable only because this is a local single-user app on localhost. |
