...
**Design tokens** (`shell/tokens.css`): ground `rgba(15,15,15,.42)` · surface `rgba(22,22,22,.72)` · elevated `rgba(28,28,28,.84)` · line `#2a2a2a` · ink `#cccccc` / dim `#8a8a8a` / faint `#6a6a6a`. The three ground tokens are translucent because the window is a macOS vibrancy material (`hudWindow`, `transparent: true`, `macOSPrivateApi: true` in `tauri.conf.json`) — the desktop shows through, blurred. The body must keep painting `--color-ground`; a transparent body shows the raw material with no tint, which reads as a bug rather than glass. The terminal paints with `allowTransparency` so it is not the one solid slab. Accents, one `--accent` per context: Faber `#e53311` · Noctua `#eca207` · Vesper `#953ead` · Maintenance `#da5b00`. Mono is Cascadia Code, sans is `system-ui`. Press Start 2P and the indigo world ramp are retired. **Border beam** (`Beam` in `Panels.tsx`, wrapping the `border-beam` package at `size="pulse-outside"`, `colorVariant="mono"`, hue-shift off): the Stats cards and each Repo module carry soft light pooling at points around the edge and blooming outward, breathing rather than travelling — the library's own component, since the motion is driven from its JavaScript and cannot be copied as a stylesheet. Silver everywhere; a repository that is a dev job's project takes the `sunset` palette held still, the nearest of the library's four to Faber's red. `pulse-outside` renders its core behind the child and relies on the child's own 1px border for the idle hairline, which our cards have. Under it a `.halo` (`tokens.css`) — one even glow the card's shape, breathing on the same cycle — fills the edges between the library's seven pools so the pulse encompasses the card; the pools stay mono, the halo carries the colour. The vibrancy is pinned to its active appearance (`windowEffects.state: "active"` in `tauri.conf.json`) so the window does not wash to grey when another app is in front. **Lifetime tokens** is one stacked bar (`Stacked` in `App.tsx`): the four kinds as shares of one total, greys darkest to lightest, a 2px floor per segment, a dot-and-figure legend beneath.
