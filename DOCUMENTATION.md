...
**The staleness pass runs first.** `staleness.flag_pass()` is nightshift's opening step: for every job folder (`modes/<dev|learn|research>/jobs/`, `maintenance/jobs/`) it marks a job `flagged` in its own `context.md` when the context is six hours untouched, the job is not at Ship or Done, and the runtime log does not end in a clean `SESSION_END`. Two logs count for one job, since a hosted session logs as `faber__<slug>` and a project's own hooks as `dev__<slug>`, and the newest line decides. The dev scan that follows reads the same folders and stages a status note for each flagged job. It ran on v1's `GET /mode/{name}` poll until the 2026-09-12 cutover deleted the route; from then to 2026-09-16 nothing called it, and the scan read the `jobs` array in `modes/dev/state.md`, which nothing had written since v1 either. The folders are the only index now; the MCP `worklist` reads them too.
