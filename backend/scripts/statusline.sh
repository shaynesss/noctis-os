...
#
# $1 is the backend port. $2 is the Noctis mode that launched the session --
# the transcript on disk never records that (its `mode` is the CLI's permission
# mode), so this is the one place the mapping is captured, the first time the
...
printf '%s' "$PAYLOAD" | curl -s -m 2 -o /dev/null \
  -X POST "http://127.0.0.1:${PORT}/v2/sessions/statusline?mode=${MODE}" \
