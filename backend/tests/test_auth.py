...
def test_wrong_token_rejected(client):
    response = client.get("/v2/config", headers={"Authorization": "Bearer wrong"})
    assert response.status_code == 401


def test_correct_token_accepted(client, auth_headers):
    response = client.get("/v2/config", headers=auth_headers)
    assert response.status_code == 200


def test_disallowed_origin_rejected(client, auth_headers):
    headers = {**auth_headers, "Origin": "http://evil.example"}
    response = client.get("/v2/config", headers=headers)
...
def test_packaged_app_origin_is_accepted(auth_headers, client):
    ...
    r = client.get("/v2/config", headers={**auth_headers, "Origin": TAURI_ORIGIN})
    assert r.status_code != 403


def test_an_unrelated_origin_is_still_refused(auth_headers, client):
    r = client.get("/v2/config", headers={**auth_headers, "Origin": "http://evil.example"})
    assert r.status_code == 403
