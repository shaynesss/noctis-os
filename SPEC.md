...
- **Learn, Research, Settings, Nightshift → macOS Terminal.app.** Launcher opens a new Terminal.app window via `osascript`, sets background to a darkened/desaturated version of the character's locked hex (HSL lightness ~15-20%, hue preserved), sets window title to `{character} — {mode} — {current job/topic}`. Terminal.app has no separate "border" — window chrome is OS-drawn — so background tint is the real equivalent of the original colored-border idea. **No `CLAUDE_CONFIG_DIR` override (changed 2026-09-11).** These four used to launch against `launch_config/nondev/` so they would not inherit Dev's methodology from the global `CLAUDE.md`; that bought isolation at the price of the whole config root, leaving a Learn session with no plugins, skills, subagents or accumulated permissions. The global `CLAUDE.md` is the universal prompt now, so the leak is gone at its source and each surface passes its own methodology, subagents and settings as flags instead.
...
- **Dev → VS Code.** Writes a `folderOpen` task that runs the real `claude` CLI in the integrated terminal, then `code --new-window <path>`. No `CLAUDE_CONFIG_DIR` override anywhere any more (2026-09-11): `~/.claude/CLAUDE.md` is the *universal* prompt, not a mode, and each surface passes its own methodology via `--append-system-prompt`. The config root is infrastructure — plugins, skills, subagents, MCP servers, permissions, memory — and every session inherits all of it. Redirecting it to give a mode a different CLAUDE.md cost the entire surface to solve a text problem.
...
- **`~/.claude/CLAUDE.md` — resolved again, differently (2026-09-11).** It pointed at `modes/dev/dev.md`, which made the *machine itself* Faber: anything reading the default config root inherited the build process. That was survivable while every Noctis surface redirected `CLAUDE_CONFIG_DIR` away from it, and became a live leak the moment they stopped — every mode loading ~8k words of spec-and-ship process it should never see. The symlink now points at `prompts/system.md`, the universal prompt: mode-agnostic vault conventions, communication rules, session-close discipline. Mode identity is passed per launch via `--append-system-prompt`, including Faber's own, so no surface depends on what the config root happens to contain. Cost, accepted deliberately: a bare `claude` in a terminal is no longer Faber by default.
