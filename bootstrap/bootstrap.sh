...
for mode in general faber noctua vesper maintenance; do
  dir="$CONFIG_ROOT/$mode"
  if [ ! -d "$dir" ]; then
    if act "create config dir for $mode"; then
      mkdir -p "$dir"
      cat > "$dir/CLAUDE.md" <<EOF
# Placeholder — $mode

Generated by bootstrap. The orchestrator rewrites this file on every launch
from \`second-brain/prompts/system.md\` plus this mode's overlay (Stage 1
item 7). Do not hand-edit: your changes are overwritten at next launch.
EOF
      ok "created config dir: $mode"
    fi
  else
    skip "config dir exists: $mode"
  fi
  # Authenticated? The hashed Keychain entry is the only reliable signal,
  # and we cannot read it, so probe the dir instead.
  if [ -f "$dir/.claude.json" ] && grep -q "oauthAccount" "$dir/.claude.json" 2>/dev/null; then
    ok "$mode — authenticated"
  else
    NEEDS_LOGIN+=("$mode")
  fi
done
...
# ── 7. hooks ──────────────────────────────────────────────────────────
head_ "7. Telemetry hooks"

# Hook commands need absolute interpreter + script paths; this is why the
# generated settings.json is machine state and never tracked.
PY_BIN="$REPO_ROOT/backend/.venv/bin/python3"
if [ ! -x "$PY_BIN" ]; then
  warn "backend venv not built yet — run 'make setup' first, then re-run this"
else
  wired_any=0
  for mode in general faber noctua vesper maintenance; do
    dir="$CONFIG_ROOT/$mode"
    if [ ! -d "$dir" ]; then
      [ "$DRY_RUN" = 1 ] && skip "$mode — would wire hooks once the config dir exists"
      continue
    fi
    wired_any=1
    want=$(cat <<EOF
{
  "hooks": {
    "PostToolUse": [{"matcher": "", "hooks": [{"type": "command", "command": "$PY_BIN $REPO_ROOT/backend/hooks/log_action.py"}]}],
    "SessionEnd": [{"matcher": "", "hooks": [{"type": "command", "command": "$PY_BIN $REPO_ROOT/backend/hooks/mark_session_end.py"}]}]
  }
}
EOF
)
    if [ -f "$dir/settings.json" ] && python3 -c "
import json,sys
a=json.load(open('$dir/settings.json')).get('hooks')
b=json.loads('''$want''').get('hooks')
sys.exit(0 if a==b else 1)" 2>/dev/null; then
      skip "$mode — hooks already wired"
    elif [ -f "$dir/settings.json" ] && [ "$FORCE" != 1 ]; then
      warn "$mode — settings.json exists with different hooks, left alone (--force to overwrite)"
    elif act "wire hooks for $mode"; then
      printf '%s\n' "$want" > "$dir/settings.json"
      ok "$mode — hooks wired"
    fi
  done
  [ "$wired_any" = 0 ] && [ "$DRY_RUN" != 1 ] && warn "no config dirs found to wire"
fi
...
if [ ${#NEEDS_LOGIN[@]} -gt 0 ]; then
  warn "${#NEEDS_LOGIN[@]} config dir(s) need a one-time interactive login:"
  for mode in "${NEEDS_LOGIN[@]}"; do
    printf "      ${DIM}CLAUDE_CONFIG_DIR=%s/%s claude${RESET}\n" "$CONFIG_ROOT" "$mode"
  done
  printf "      ${DIM}…then /login in each. Credentials are Keychain entries keyed to the\n      dir path, so this cannot be scripted or copied between dirs.${RESET}\n"
fi
...
[ ${#NEEDS_LOGIN[@]} -gt 0 ] && printf " ${YELLOW}(logins still outstanding)${RESET}"
printf "\n"
