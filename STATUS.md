...
- **Settings** — the universal prompt and each mode's overlay, edited in the vault in place (uncommitted; Repo commits them), and the **regression suite**, run from the card scoped to what an edit affects, each case recorded against the prompt it ran on.
