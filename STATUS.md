...
- **Brief** — served from the vault when written; the scheduler that writes it is the one unbuilt piece.
...
1. **The scheduler (launchd on wake).** `brief/generate.py` writes `brief/today.md`; nothing fires it, so the brief only appears when asked for. The last piece of item 6.
2. **The brief page cannot tell stale from current.** Until the scheduler exists, whatever was last written renders as if it were today's. A staleness indicator and a regenerate control are worth having regardless, so the scheduler's first failure is not silent.
